...
import {DeckHTMLFixture} from "./Deck/deckHTML.fixture";
...
export class CardFixture implements fixture {
    ...
    public async fillTable(): Promise<void> {
        ...
        const deckHTML = new DeckHTMLFixture();
        ...
        await Promise.all([
            ...
            deckHTML.cardsInstance = await Promise.all(deckHTML.cards.map(async (card) => await manager.card.create(card)))
        ]);
        ...
        for (let i = 0; i < deckPython.cardsInstance.length; i += 2) {
            await Promise.all([
                deckHTML.cardsInstance[i].setDeck(deckFixture.deck_html),
                deckHTML.cardsInstance[i + 1].setDeck(deckFixture.deck_html),
                await deckHTML.cardsInstance[i].setCardAssociate(deckHTML.cardsInstance[i + 1]),
                await deckHTML.cardsInstance[i + 1].setCardAssociate(deckHTML.cardsInstance[i])
            ]);
        }
    }
